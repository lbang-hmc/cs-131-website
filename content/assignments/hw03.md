...
!!! note "How you'll get materials and submit this assignment"
    Getting the starter code and turning in your work follows the same process as every homework this semester — see [**The Homework Workflow**](../how-to/homework-workflow.md) for the full walkthrough. In short: from the [`hw3` template repo](https://github.com/hmc-cs-131-fa-2026/hw3), make your own private copy, clone it to the course server, and submit your test output and code on Gradescope. The rest of this page covers what the assignment actually asks of you.
...
- [ ] Get the assignment materials (see [The Homework Workflow](../how-to/homework-workflow.md))
...
Submit on [Gradescope](https://www.gradescope.com), under **HW 03: Lists, Datatypes, and a Stack Machine**:

1. Run the full test suite one last time: `runhaskell -itest test/Spec.hs`.
2. Copy the **entire** terminal output and paste it into the HW3 text box.
3. Upload your `HW3.hs` to the same assignment (for our records).
...
The stack machine you just built is a tiny example of a much bigger idea: **[stack-oriented programming](https://en.wikipedia.org/wiki/Stack-oriented_programming)**, where the stack isn't just an implementation detail — it *is* the language. Forth, PostScript (the language your printer probably speaks), and countless calculators and virtual machines (including the JVM and Python's own bytecode interpreter) are all built this way. It's not just a practical trick, either — it's theoretically rich territory: the language [Joy](https://hypercubed.github.io/joy/html/j02maf.html) treats stack-based programs as literal compositions of functions, with a clean underlying algebra (no variables, no substitution — just functions built out of other functions). Worth a rabbit hole if stack machines caught your interest.
