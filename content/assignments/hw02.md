...
!!! note "How you'll get materials and submit this assignment"
    Getting the starter code and turning in your work follows the same process as every homework this semester — see [**The Homework Workflow**](../how-to/homework-workflow.md) for the full walkthrough. In short: from the [`hw2` template repo](https://github.com/hmc-cs-131-fa-2026/hw2), make your own private copy, clone it to the course server, and submit your test output and code on Gradescope. The rest of this page covers what the assignment actually asks of you.
...
- [ ] Get the materials for the assignment (see [The Homework Workflow](../how-to/homework-workflow.md))
- [ ] Implement `fib`
...
- [ ] Turn in the assignment (see [The Homework Workflow](../how-to/homework-workflow.md))

---

## Preparing for the Assignment

Before diving into the code, take some time to get set up — this is worth doing properly, and we've built time for it into the assignment. Do your work on the course server, where Haskell and the test libraries are already installed. The [How-To Guides](../how-to/index.md) cover [connecting with VS Code](../how-to/connecting-with-vscode.md), the [homework workflow](../how-to/homework-workflow.md), and [how modules and tests work](../how-to/writing-and-running-tests.md).
...
You'll get the starter files from the [`hw2` template repo](https://github.com/hmc-cs-131-fa-2026/hw2): make your own private copy and clone it to the course server. [The Homework Workflow](../how-to/homework-workflow.md) walks through every step. Your job is to fill in the definitions marked `undefined` in `Arithmetic.hs` and `Functions.hs` (plus adding the `intfib` tests described below); the starter code's names and types are fixed, but you can always add helper functions.

Unlike HW1, you can work with a partner on this assignment — remember to follow the pair-programming rules from the [syllabus](../syllabus.md). If you pair up, add your partner as a collaborator on your repo (also covered in that guide).
...
Prefer an interactive session? `ghci test/FunctionSpec.hs`, then type `main` at the prompt (`:r` then `main` after each edit, `:quit` when done). See [Writing and Running Tests](../how-to/writing-and-running-tests.md) for the full picture.
...
Submit on [Gradescope](https://www.gradescope.com), under **HW 02: FUNctional Programming**:

1. Run the full test suite one last time: `runhaskell -itest test/Spec.hs`.
2. Copy the **entire** terminal output and paste it into the HW2 text box.
3. Upload your `Arithmetic.hs` and `Functions.hs` to the same assignment (for our records).

[The Homework Workflow](../how-to/homework-workflow.md) covers the submission step in context. There's also a short **HW 2 reflection** to submit separately on Gradescope.
...
- Addition, multiplication, and exponentiation are all specific instances of the [*hyperoperation*](https://en.wikipedia.org/wiki/Hyperoperation) sequence, which is closely related to the [Ackermann function](https://en.wikipedia.org/wiki/Ackermann_function) — one of the classic examples of a function that is computable but not primitive recursive. Can you define the hyperoperation itself in Haskell, and then define `addition`, `multiplication`, and `exponentiation` as calls to it?
- If you got all three working, try implementing the next operation in the sequence: *tetration* (repeated exponentiation). It grows fast — `tetration 2 4` is already `65536`.
